...
public class PlayerController : MonoBehaviour
{
            ...
            for (int i = 0; i < enemies.Length; i++)
            {
                if (enemies[i] != null)
                {
                    ...
                    if(awayFromPlayer.magnitude < distance)
                    {
                        ...
                        enemyRb.AddForce(2 * powerupStrength * awayFromPlayer, ForceMode.Impulse);
                    }
                }
            }
        }
    ...
    private void Smash()
    {
        if (isOnIsland)
        {
             playerRb.AddForce(jumpSpeed * Vector3.up, ForceMode.Impulse);
    ...
    IEnumerator SmashRoutine()
    {
        yield return new WaitForSeconds(0.3f);
         ...
         playerRb.AddForce(2 * jumpSpeed * Vector3.down, ForceMode.Impulse);
        isSmashed = true;
    }
}
